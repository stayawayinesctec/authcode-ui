<template>
  <header class="bg-gray-900 sm:flex sm:justify-between sm:items-center sm:px-4 sm:py-3">
    ...
    <nav :class="isOpen ? 'block' : 'hidden'" class="px-2 pt-2 pb-4 sm:flex sm:p-0">
      <a @click="logout" class="block px-2 py-1 text-white font-semibold rounded hover:bg-gray-800">Logout</a>
    </nav>
  </header>
</template>
...
export default {
  ...
  methods: {
      logout: function() {
          this.$auth.logout().then(() => this.$router.push({ name: 'auth-login' }));
      }
  }
}
</script>